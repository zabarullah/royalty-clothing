...
const CustomButton = ({ children, isGoogleSignIn,...otherProps }) => (
    <button className={`${isGoogleSignIn ? 'google-sign-in' : ''} custom-button`} {...otherProps}> {/* the props children here are what ever will be between the button tags, is googleSignIn is passed from the Sign-in component and so are the ...otherprops which refer to onClick method  or the onSubmit function, etc. If isGoogleSignIn is true then use Google-sign-in class otherwise empty string, and in all cases the custom-botton class will be used*/}
        {children}      
    </button>
);
